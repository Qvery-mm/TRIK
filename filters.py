...
class Program():
    ...
    def execMain(self):
        ...
        while True:
            ...
            array.append(dist)
            o_array_1 = self.filter.confidence_interval(array, 30, 70)
            o_array_2 = self.filter.sliding_mean(array, 5)
            o_array_3 = self.filter.sliding_median(array, 5)
            o_array_4 = self.filter.weighted_sliding_average(array, 5)
            o_array_5 = self.filter.EMA(array, 0.7)
            

            self.robot.plot(o_array_1)
            
            script.removeFile("plots.csv")

            
            
            for i in array:
              script.writeToFile("plots.csv", str(i) + ";")
            script.writeToFile("plots.csv", "\n")
            for i in o_array_1:
              script.writeToFile("plots.csv", str(i) + ";")
            script.writeToFile("plots.csv", "\n")
            for i in o_array_2:
